const methods = [
...

  {
    ...
    description:
      "This function is responsible for finding the event with the specified parameter and value, and then replacing the values for that parameter. For example, if we insert 'id' in the parameter, 5 in the value, and {start: startDate, end: endDate, data: {employees: []}} in the object, then in the event with id 5, the start and end dates will be changed, and the object data of any type will be assigned a list of employees.",
  },
  {
    id: 1,
    name: "",
  ...
  },
  {
    ...
    description:
      "This function changes the threshold responsible for the threshold during the dragging of events. Read more in 'params'.",
  },
  {
    id: 9,
    name: "Set renderer ",
    ...
    description:
      "This function changes the renderer, a parameter responsible for substituting the installed component into event elements instead of the default event backdrop.",
  },
  {
    id: 10,
    name: "Set event template ",
    call: "setEventTemplate",
    return: "void",
    code:
      "    api.setEventTemplate((event:VuelCalendarEvent)=>{\n" +
      '      return `<div style="background: aqua; height:100%">\n' +
      '                <p style="height:40px; padding:0;margin:0; color:red">${event.label}</p>\n' +
      "            </div>`\n" +
      "    })",
    args: "eventTemplate: EventTemplate",
    description:
      "This function changes the eventTemplate, a parameter responsible for substituting the HTML DOM tree into event elements instead of the default event backdrop.",
  },
];
const getFunctionsMenu = (router: any) => {
  const menu = [];
  for (const func of methods) {
    menu.push({
      ...
      command: () =>
        router.push({ name: "ApiFunctions", query: { docs: func.call } }),
    });
  }
  return menu;
};
export { methods, getFunctionsMenu };
