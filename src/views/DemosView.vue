...
const _resizableEvents = resizableEvents(getAllowResize, setResponse, setResizeApi)
...
<template>
  <section class="grid grid-cols-1 lg:grid-cols-6 gap-5">
    ...
    <div class="col-span-5">
      <h1 class="text-3xl mt-4" id="renderer"># Event renderer / <span id="event-template">Event Template</span></h1>
      <p class="mb-2">Select renderer to change.</p>
      <div class="flex gap-5 items-center">
        <p>Renderer: </p><Dropdown v-model="renderer" :options="rendererDrops" @update:model-value="changeRenderer"/>
      </div>
      <div>
      ...
      <div class="flex gap-5 items-center lg:flex-row flex-col  mb-2 lg:mb-0">
        <p>Allow drop: </p><Checkbox v-model="allowDrop" :binary="true" />
        <p>Treshold:</p><InputNumber :min="0" :max="59" class="h-[30px]" v-model="tresholdDrop" @update:model-value="setTresholdDrop"/>
      </div>
      <div>
      ...
      <p class="mb-2">Click on the starting or ending edge of the event and drag.</p>
      <div class="flex gap-5 items-center lg:flex-row flex-col  mb-2 lg:mb-0">
        <p>Allow resize: </p><Checkbox v-model="allowResize" :binary="true" />
        <p>Treshold:</p><InputNumber :min="0" :max="59" class="h-[30px]" v-model="tresholdResize" @update:model-value="setTresholdResize"/>
      </div>
      <div>
      ...
      <div class="flex gap-5 items-center lg:flex-row flex-col  mb-2 lg:mb-0">
        <p>(min:0, max:24) Start hour:</p><InputNumber :min="0" :max="24" class="h-[30px]" v-model="startHour" @update:model-value="setTimeRange"/>
        <p>(min:0, max:24) End hour:</p><InputNumber :min="0" :max="24" class="h-[30px]" v-model="endHour" @update:model-value="setTimeRange"/>
      ...
      <div class="flex gap-5 items-center lg:flex-row flex-col  mb-2 lg:mb-0">
        <p>Start date:</p><Calendar  class="h-[30px]" date-format="yy-mm-dd" v-model="startDate" @update:model-value="setDateRange"/>
        <p>End date:</p><Calendar class="h-[30px]" date-format="yy-mm-dd" v-model="endDate" @update:model-value="setDateRange"/>
      </div>
      <div>
      ...
      <div class="flex gap-5 items-center">
        <Button  @click="switchView" class="mb-2" severity="success">Switch view</Button>
      </div>
      <div>
